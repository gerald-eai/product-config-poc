...
class SystemMapForm:
    ...
    def base_form(self, default_vals: dict):
        ...
        form_inputs["area_name"] = form.text_input(
            "Area Name", default_vals["area_name"]
        )
        form_inputs["region_name"] = form.text_input(
            "Region Name", default_vals["region_name"]
        )
        form_inputs["odmt_area_id"] = form.number_input(
            "ODMT Area ID", default_vals["odmt_area_id"]
        )
    ...
    def create_current_entry_form(self, default_vals: dict):
        st.info(f"You are creating a new entry in the live System Map Table")

        form, form_inputs = self.base_form(default_vals=default_vals)
        submit_form = form.form_submit_button("Create New System Mapping Entry")
        ...
        if submit_form:
            ...
            try:
                ...
                response = self.api_session.create_new_entry(
                    "system-mapping/live/", req_body=create_live_sys_map
                )
    ...
    def create_staged_entry_form(self, default_vals: dict):
        st.info(
            f"You are editing data for the following Hydraulic System: {default_vals['hydraulic_system_name']}"
        )
        form, form_inputs = self.base_form(default_vals=default_vals)
        ...
        if submit_form:
            ...
            try:
                st.write("Submitting Data")
                # :TODO: Let's make a comparison of the data and only update the necessary fields
                create_sys_map_update = SystemMapRequest.CreateNewStagedEntry(
                    **form_inputs
                )
                ...
                response = self.api_session.create_staged_entry(
                    "system-mapping/updates/", req_body=create_sys_map_update
                )
                print(f"Here is your damned response! {response}")
    ...
    def edit_staged_entry_form(self, default_vals: dict):
        st.info(
            f"You are editing data for the following staged_update: {default_vals['id']}"
        )
        form, form_inputs = self.base_form(default_vals=default_vals)
        ...
        if submit_form:
            ...
            try:
                ...
                response = self.api_session.edit_staged_entry(
                    f"system-mapping/updates/{edit_staged_sys_map.id}",
                    req_body=edit_staged_sys_map,
                )
                print(f"Here is your damned response! {response}")
